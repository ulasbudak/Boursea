...
- **Given** `apps/web` dizini, **When** `pnpm --filter web dev` çalıştırılırsa, **Then** Next.js geliştirme sunucusu başlar ve tarayıcıda "borsa_app" yazan bir placeholder ana sayfa görüntülenir.
- **And** `pnpm --filter web build` hatasız tamamlanır (production build).

**AC3 — Mobil uygulama ayağa kalkar**
- **Given** `apps/mobile` dizini, **When** `pnpm --filter mobile start` (Expo) çalıştırılırsa, **Then** Expo geliştirme sunucusu başlar ve bir simülatör/Expo Go üzerinde "borsa_app" yazan bir placeholder ekran görüntülenir.
