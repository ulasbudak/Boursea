...
- **Prod bellek düzeltmesi (2026-09-25):** Render (512 MB) ilk teknik rapor isteğinde "memory limit exceeded" ile yeniden başladı — `ultralytics` + `torch` yüklenince süreç ~800 MB'a çıkıyordu. Model artık ONNX Runtime ile çalışıyor (`ultralytics`/`torch` bağımlılıktan çıkarıldı; ONNX dosyası `scripts/export_chartscan_onnx.py` ile üretilip `chartscan-yolov8-onnx-v1` GitHub release asset'i olarak SHA-256 sabitli indiriliyor). Ön/son işleme (rect letterbox + sınıf bazlı NMS) ultralytics `predict()` ile birebir aynı sonucu verecek şekilde yazıldı — 8 sentetik grafikte tüm tespitler ve güven değerleri eşleşti. Ayrıca her rapor başına kapatılmayan bir matplotlib figürü sızıntısı giderildi ve eşzamanlı inference'lar bir kilitle seri hale getirildi. Ölçüm (macOS, tam uygulama yüklü): tepe ~410–490 MB, torch hiç yüklenmiyor.
