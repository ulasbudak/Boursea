...
- [x] **Web görsel doğrulama** (2026-09-21) — Claude tarafından, headless Chromium (Playwright) ile: `/settings`'te "Technology" sektörü seçildi, dashboard'daki Highlights bölümünde bu sektörün gerçek günün en çok hareket eden hisseleri (AMAT +6.5%, QCOM -5.8%, MU +3.9% vb.) doğru göründüğü doğrulandı. **Bulgu ve düzeltme:** seçim yapıldıktan hemen sonra, aynı oturumda dashboard'a gidildiğinde değişiklik yansımıyordu (stale JWT — bkz. Teknik Notlar); `setInterestSectors()`'a `supabase.auth.refreshSession()` eklenerek düzeltildi ve aynı gün canlı olarak yeniden doğrulandı: aynı oturumda "Healthcare"e geçildi, hiç çıkış yapılmadan dashboard'a gidildiğinde Highlights anında gerçek sağlık sektörü hisseleriyle (ISRG, CVS, AMGN, TMO, MDT, GILD, UNH, ABT, BMY, DHR) güncellendi.
...
- **Düzeltilen gecikme (bulundu ve düzeltildi 2026-09-21):** `setInterestSectors()` server action'ı `supabase.auth.updateUser({ data: { interest_sectors } })` çağırıp `revalidatePath("/", "layout")` yapıyordu — bu, Next.js'in sayfa önbelleğini geçersiz kılıyor ama **mevcut oturumun access token'ındaki `user_metadata` claim'ini yenilemiyordu**. Dashboard sunucu bileşeni `getClaims()` ile o token'ı okuduğundan, kullanıcı sektör seçip hemen dashboard'a dönerse eski (seçimsiz) hâli görüyordu. **Düzeltme:** `apps/web/src/app/settings/actions.ts::setInterestSectors()`'a `updateUser()` sonrası bir `await supabase.auth.refreshSession()` çağrısı eklendi — bu, yeni bir access token alıp `@supabase/ssr`'ın cookie adaptörü üzerinden kalıcı hale getiriyor (server action içinde çalıştığından cookie yazımı izinli, Server Component'lerin aksine). Canlı doğrulandı (bkz. yukarı). **Kapsam dışı bırakılan:** Story 1.3'teki `setLocale()` aynı `updateUser()` deseninini kullanıyor ama locale render'ı zaten ayrı bir cookie'den (`borocean_locale`) okunuyor, JWT'den değil — o yüzden aynı gecikme kullanıcıya görünür değil, dokunulmadı.
