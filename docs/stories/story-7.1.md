...
- [x] **Web görsel doğrulama** (2026-09-21) — Claude tarafından, headless Chromium (Playwright) ile: `/settings`'te "Technology" sektörü seçildi, dashboard'daki Highlights bölümünde bu sektörün gerçek günün en çok hareket eden hisseleri (AMAT +6.5%, QCOM -5.8%, MU +3.9% vb.) doğru göründüğü doğrulandı. **Bulgu:** seçim yapıldıktan hemen sonra, aynı oturumda dashboard'a gidildiğinde değişiklik yansımadı ("ayarlardan ilgi alanı sektörü seç" mesajı devam etti) — yalnızca çıkış yapıp tekrar giriş yaptıktan sonra (yeni JWT ile) Highlights güncellendi. Kök neden ve önerilen düzeltme için bkz. Teknik Notlar.
...
- **Bilinen gecikme (2026-09-21'de canlı doğrulamada bulundu):** `setInterestSectors()` server action'ı `supabase.auth.updateUser({ data: { interest_sectors } })` çağırıp `revalidatePath("/", "layout")` yapıyor — bu, Next.js'in sayfa önbelleğini geçersiz kılıyor ama **mevcut oturumun access token'ındaki `user_metadata` claim'ini yenilemiyor**. Dashboard sunucu bileşeni `getClaims()` ile o token'ı okuduğundan, kullanıcı sektör seçip hemen dashboard'a dönerse eski (seçimsiz) hâli görebilir; değişiklik ancak token doğal olarak yenilendiğinde (veya kullanıcı çıkış/giriş yaptığında) yansıyor. Story 1.3'teki `locale` güncellemesi de aynı deseni kullanıyor, muhtemelen aynı gecikmeye tabi. **Olası düzeltme:** `updateUser()` sonrası `supabase.auth.refreshSession()` çağırıp yeni token'ı cookie'ye yazmak, ya da settings sayfasında "kaydedildi, birkaç dakika içinde yansır" gibi bir not eklemek — bu story'nin kapsamında düzeltilmedi, yalnızca tespit edilip belgelendi.
